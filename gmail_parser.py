...
# Modified 2009 Robert Fletcher <lobatifricha@gmail.com>
...
def readmail(feed, maxlen):
    ...
    if len(atom.entries) > 0:
        print '${color1}${font} %s new email(s)\n' % (len(atom.entries))
    for i in range(min(len(atom.entries), maxlen)):
#       print '${color2}%s ${color1}From:${color2} %s' % (atom.entries[i].title, atom.entries[i].author_detail.name)
        print '${color2}%s' % atom.entries[i].title
# uncomment the following line if you want to show the name of the sender
        print '${color3}    ~%s' % atom.entries[i].author_detail.name
